...
namespace TestingJsonWebKeys
{
    public class RsaExample
    {
        ...
        public static void Run()
        {
            ...
            // =================== criação do Json Cabeçalho
            var cabecalho = new List<System.Security.Claims.Claim>
            {
            ...
            JwtPayload payloadcabecalho = new JwtPayload(cabecalho);
            var jsoncabecalho = payloadcabecalho.SerializeToJson();

            // ==================================================================================

            // =================== criação do Json Payload
            ...
            //================ Cabeçalho em Base64url  =================================================
            string base64cabecalho = Convert.ToBase64String(Encoding.UTF8.GetBytes(jsoncabecalho));
            //=======================================================

            // ====================== Payload em Base64url ========================================
            string base64payload = Convert.ToBase64String(Encoding.UTF8.GetBytes(jsonPayload));

            // ================= byte array {Cabeçalho em Base64url}.{Payload em Base64url} ========================
            byte[] bytes = Encoding.ASCII.GetBytes(base64cabecalho + "." + base64payload);

            // ===============================================================================================

            // convert byte array {Cabeçalho em Base64url}.{Payload em Base64url}´para string
            ...
            var lastJws = tokenHandler.CreateToken(result, Jwt.SigningCredentials);
